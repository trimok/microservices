...
public class ControllerTest {
    ...
    private Patient getPatientFromMvcResult(MvcResult mvcResult)
    ...
    private List<Patient> getListPatientFromMvcResult(MvcResult mvcResult)
	    throws UnsupportedEncodingException, JsonMappingException, JsonProcessingException {
	String json = mvcResult.getResponse().getContentAsString();
	...
	List<Patient> patientsReturn = getListPatientFromMvcResult(mvcResult);
	...
	Patient patient = getPatientFromMvcResult(mvcResult);
    ...
    @Test
    public void getPatient_PatientNoContentException() throws Exception {
	...
	MvcResult mvcResult = mockMvc
		.perform(MockMvcRequestBuilders.post("/patient").content(mapper.writeValueAsString(patient))
			.contentType(MediaType.APPLICATION_JSON)
			.accept(MediaType.APPLICATION_JSON))
		.andExpect(status().is(201)).andReturn();

	Patient patientSaved = getPatientFromMvcResult(mvcResult);
	...
	MvcResult mvcResult = mockMvc
		.perform(MockMvcRequestBuilders.put("/patient").content(mapper.writeValueAsString(patientDatabase))
			.contentType(MediaType.APPLICATION_JSON)
			.accept(MediaType.APPLICATION_JSON))
		.andExpect(status().is(200)).andReturn();

	Patient patientUpdated = getPatientFromMvcResult(mvcResult);
	...
	mockMvc
		.perform(MockMvcRequestBuilders.put("/patient").content(mapper.writeValueAsString(patient))
			.contentType(MediaType.APPLICATION_JSON)
			.accept(MediaType.APPLICATION_JSON))
		.andExpect(status().is(204)).andReturn();
